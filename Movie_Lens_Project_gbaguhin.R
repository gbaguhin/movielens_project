...
# Let's query how many movie titles had 10 or less number of ratings
...
# Let's query the number of users who rated 10 or less movies:
...
# Use cross-validation to choose optimum value.
...
# Despite limitations due to the sparseness of ratings empirical data to work with which could result in noisy estimates that
# need to be attenuated, we were able to model in different stages recommendation system algorithms using our training data.
...
# As we earlier mentioned, our training data included users rating less than 10 movies and movies that had just 10 ratings. We introduced regularization where a penalty term lambda is added to constrain the effects of
# these data on the RMSE value.Using the penalty term on the movie effect model did not significantly improve the RMSE did
# improve the RMSE value to 0.865 when used with least squares estimate of Movie + User effect model. This is an
# improvement of 18.5%.
...
# ability to detect and stop fraud. 
